...
export const EXTENSION_LESSONS: InteractiveLesson[] = [
  {
    ...
    steps:[
      ...
      {kind:'predict',prompt:'A card claims a 1M-token context.',questions:[{label:'what must still be tested',options:['Recall quality, latency, memory cost, and behavior across positions','Whether token IDs are integers','Whether softmax sums to one'],answer:0,reveal:'Maximum accepted length is only one dimension of long-context usefulness.'}]},
    ],
  },
  {
    ...
    steps:[
      ...
      {kind:'predict',prompt:'A 48-layer hybrid uses 12 full-attention layers and 36 fixed-state layers.',questions:[
        {label:'growing KV-cache layers',options:['12','36','48'],answer:0,reveal:'Only full-attention layers store per-position keys and values; fixed-state layers retain bounded state.'},
