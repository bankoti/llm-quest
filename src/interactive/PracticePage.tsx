...
import type { InteractiveLesson, McqStep, PredictStep } from './types'
import { StepMcq, StepPredict } from './InteractiveLessonPage'

interface PracticeItem {
  lesson: InteractiveLesson
  stepIdx: number
  step: McqStep | PredictStep
}
...
function buildQueue(): PracticeItem[] {
  ...
  for (const lesson of order) {
    ...
    for (const stepIdx of [...missed].sort((a, b) => a - b)) {
      ...
      if (step && (step.kind === 'mcq' || step.kind === 'predict')) q.push({ lesson, stepIdx, step })
    }
  }
  return q
}
