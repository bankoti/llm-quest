...
export const INTERACTIVE_LESSONS: InteractiveLesson[] = [
  ...
  // ── L2: Tokenization ──────────────────────────────────────────────────────
  {
    ...
    steps: [
      ...
      {
        kind: 'mcq',
        prompt: 'BPE starts with individual characters and merges the most frequent adjacent pair each round. Commit before the demo: what does a larger vocabulary mean?',
        options: [
      ...
      { kind: 'widget', widget: TokenFailPlay },
      {
        ...
        whys: [
          undefined,
          'Attention can count things it can see — tokens, positions. The letters never reach it in the first place.',
          'Base models fail this too, before any RLHF. The blindness is at the input, not the fine-tune.',
          '"strawberry" is common enough to be in every vocabulary — that is exactly the problem: it arrives fused as one opaque ID.',
        ],
  ...
  // ── L3: Attention ─────────────────────────────────────────────────────────
  {
    ...
    steps: [
      ...
      },
      {
        ...
        whys: [
          undefined,
          'Softmax already normalizes to sum 1 — no extra scaling is needed for that.',
          'Values are multiplied after softmax; their scale never mixes with the scores.',
          'It is math, not hardware: the constant tames dot-product variance before softmax.',
        ],
  ...
  // ── Inside the Transformer ────────────────────────────────────────────────
  {
    ...
    steps: [
      ...
      {
        kind: 'mcq',
        prompt: 'Commit before the demo. A transformer with no positional encoding reads "dog bites man" and "man bites dog". What does it compute?',
        options: [
        ...
        nudge: 'Attention is a weighted sum over token content. Does a sum care about where a token sits?',
      },
      ...
      { kind: 'widget', widget: CausalMaskPlay },
      {
        ...
        options: [
          'Training feeds the whole sequence at once, so every position predicts in parallel and the future is physically present in the tensor \u2014 without the mask each position could copy its answer from the future',
        ...
        whys: [
          undefined,
          'Sequences arrive intact; shuffling happens across examples, not within one. Order inside a sequence is never lost.',
          'The mask adds a T×T matrix — it costs memory rather than saving it.',
          'It is not statistical regularization; without it, position t can literally read token t+1 and training collapses into copying.',
        ],
    ...
    steps: [
      {
        ...
        cta: 'Diagnose a training run',
      },
      {
        kind: 'mcq',
        prompt: 'Commit before the demo. Loss is stuck at the same value for 1000 steps, barely moving. What is the most likely cause?',
        options: [
        ...
        whys: [
          undefined,
          'Too-large LR shows up as spikes, oscillation, or divergence — not a flat line.',
          'Batch-size parity is a myth; nothing in the math cares whether it is odd or even.',
          'Global convergence after 1000 steps of a deep network is wishful thinking — check the LR first.',
        ],
      ...
      {
        kind: 'numeric',
        prompt: 'A 10-token sequence. The model sees tokens 1–9 and predicts the next token at each position. Count the loss terms and type the number.',
        code: '# input:  tokens[0:9]   — positions 0 through 8\n# target: tokens[1:10]  — the next token at each position',
        questions: [
          { label: 'Number of (prediction, target) pairs', answer: 9, tolerance: 0,
  ...
  // ── L5: Sampling ──────────────────────────────────────────────────────────
  {
    ...
    steps: [
      ...
      },
      {
        ...
        whys: [
          undefined,
          'Seeds pin the RNG, but bit-level numeric drift across hardware and library versions still breaks exact reproduction.',
          'Top-p = 1.0 disables the nucleus cutoff — you are still sampling from the full distribution.',
          'Top-k with T > 0 still draws randomly among the k candidates.',
        ],
  ...
  // ── L6: Efficiency ────────────────────────────────────────────────────────
  {
    ...
    steps: [
      ...
      { kind: 'widget', widget: PrecisionPlay },
      {
        ...
        whys: [
          undefined,
          'Full recomputation is exactly what the cache exists to avoid.',
      ...
      },
      {
        kind: 'numeric',
        ...dailyPick([
          { prompt: 'T = 2048, 32 layers, 32 K/V heads, head dim D = 128, fp16 (2 bytes). Compute the KV-cache size in GB.',
            questions: [{ label: '2 × layers × heads × T × D × bytes', answer: 1.07, tolerance: 0.15, unit: 'GB',
              reveal: '2×32×32×2048×128 = 537M values, ×2 bytes ≈ 1.07 GB. Scale T to 32k and it becomes 17 GB — the cache, not the weights, is what kills long context on consumer GPUs.' }] },
          { prompt: 'T = 8192, 32 layers, 8 K/V heads (GQA), head dim D = 128, fp16 (2 bytes). Compute the KV-cache size in GB.',
            questions: [{ label: '2 × layers × heads × T × D × bytes', answer: 1.07, tolerance: 0.15, unit: 'GB',
              reveal: '2×32×8×8192×128×2 ≈ 1.07 GB. GQA (8 heads vs 32) cuts the cache 4× — same size as a full-head model at one quarter the context.' }] },
          { prompt: 'T = 4096, 40 layers, 40 K/V heads, head dim D = 128, fp16 (2 bytes). Compute the KV-cache size in GB.',
            questions: [{ label: '2 × layers × heads × T × D × bytes', answer: 3.36, tolerance: 0.3, unit: 'GB',
              reveal: '2×40×40×4096×128×2 ≈ 3.36 GB. A 13B-class model at medium context — the cache alone rivals the VRAM left over after loading the weights.' }] },
        ] as const),
      },
    ],
  },
  ...
  // ── L7: Alignment ─────────────────────────────────────────────────────────
  {
    ...
    steps: [
      ...
      { kind: 'widget', widget: CalibrationPlay },
      {
        ...
        whys: [
          undefined,
          'The knowledge survives — probe the base distribution and calibration is still there. Only the stated confidence shifted.',
          'The reward model shapes preferences during training; it does not touch logits at inference.',
          'Nothing mathematical forbids a helpful, calibrated model — the miscalibration is a training artifact, not a theorem.',
        ],
  ...
  // ── L10: Mixture of Experts ───────────────────────────────────────────────
  {
    ...
    steps: [
      ...
      { kind: 'widget', widget: MoePlay },
      {
        ...
        whys: [
          undefined,
          '1/8 would be top-1 routing — this router picks two experts per token.',
          '1/2 would be top-4 of 8. Count again: 2 of 8.',
          'Reweighting all experts would cost the same as a dense layer — the whole point of MoE is that unpicked experts never run.',
        ],
  ...
  // ── L11: Retrieval & RAG ──────────────────────────────────────────────────
  {
    ...
    steps: [
      ...
      {
        kind: 'numeric',
        prompt: 'Context budget arithmetic. Window = 8192 tokens; system prompt + question + answer reserve 1200; each retrieved chunk is 600 tokens. Compute the answer.',
        questions: [
          { label: 'Maximum number of chunks k that fit', answer: 11, tolerance: 0,
  ...
  // ── L12: Fine-Tuning & LoRA ───────────────────────────────────────────────
  {
    ...
    steps: [
      ...
      { kind: 'widget', widget: LoraPlay },
      {
        ...
        whys: [
          undefined,
          'The base stays frozen in LoRA — that is the design; only B and A receive gradients.',
          'Rank 8 stores two thin matrices, a tiny fraction of one weight copy — nothing like 8 full copies.',
          'Optimizers do not compress gradients to a fixed rank; the low-rank structure is in the update itself, not the tooling.',
        ],
  ...
  // ── L13: Speculative Decoding & Test-Time Compute ─────────────────────────
  {
    ...
    steps: [
      ...
      {
        kind: 'numeric',
        prompt: 'Draft proposes 4 tokens per round; the target accepts each with probability 0.8 (independent, stop at first reject). Compute the expected yield: sum the acceptance probabilities 0.8 + 0.8² + 0.8³ + 0.8⁴, then add the 1 token the target supplies itself.',
        questions: [
          { label: 'Expected tokens emitted per target pass', answer: 3.36, tolerance: 0.15, unit: 'tokens',
            reveal: 'E = 0.8 + 0.64 + 0.512 + 0.41 ≈ 2.36 accepted, +1 from the target ≈ 3.36 tokens per big-model pass, versus 1 for sequential decoding. Acceptance rate is the whole game: a well-matched draft model is what makes it pay.' },
        ],
      },
    ],
  },
  ...
  // ── L15: Scaling Laws ────────────────────────────────────────────────────
  {
    ...
    steps: [
      ...
      { kind: 'widget', widget: ScalingPlay },
      {
        ...
        whys: [
          undefined,
          '300B is the pre-Chinchilla habit (Gopher-style) — exactly the undertraining the paper exposed.',
          '70B tokens would be a 1:1 ratio — an order of magnitude undertrained.',
          '10T is deliberate Llama-style overtraining for inference, not the compute-optimal point.',
        ],
      ...
      },
      {
        kind: 'numeric',
        ...dailyPick([
          { prompt: 'A 30B model was trained on 300B tokens. Chinchilla says D* = 20N. Compute the optimal token count, then compare.',
            questions: [{ label: 'Compute-optimal training tokens D*', answer: 600, tolerance: 0, unit: 'B tokens',
              reveal: '20×30B = 600B optimal. It got 300B — only half. Undertrained: spare capacity never filled.' }] },
          { prompt: 'A 7B model was trained on 1T tokens. Chinchilla says D* = 20N. Compute the optimal token count, then compare.',
            questions: [{ label: 'Compute-optimal training tokens D*', answer: 140, tolerance: 0, unit: 'B tokens',
              reveal: '20×7B = 140B optimal. It got 1T — 7× more, deliberately overtrained (Llama-style) for cheap inference.' }] },
          { prompt: 'A 13B model was trained on 260B tokens. Chinchilla says D* = 20N. Compute the optimal token count, then compare.',
            questions: [{ label: 'Compute-optimal training tokens D*', answer: 260, tolerance: 0, unit: 'B tokens',
              reveal: '20×13B = 260B — exactly what it got. Compute-optimal: best quality for this training budget.' }] },
        ] as const),
      },
    ],
  },
  ...
  // ── L17: Decoding Strategies ──────────────────────────────────────────────
  {
    ...
    steps: [
      ...
      },
      {
        ...
        whys: [
          undefined,
          'Beam search is not sampling, so temperature is not the lever — it maximizes log-probability regardless.',
          'Beam width limits how many partial sequences survive, not which vocabulary tokens are considered.',
          'That describes top-k sampling. Beam search keeps whole candidate sequences, and its blandness comes from maximizing probability.',
        ],
  ...
  // ── L18: Generalisation ───────────────────────────────────────────────────
  {
    ...
    steps: [
      ...
      {
        kind: 'mcq',
        prompt: 'Commit before the demo. Train loss: 0.3, val loss: 1.8, gap widening each epoch. The model has:',
        options: [
  ...
  // ── L19: Agents & Tool Use ────────────────────────────────────────────────
  {
    ...
    steps: [
      ...
      {
        kind: 'numeric',
        prompt: 'An agent loop: up to 10 turns. Each turn: thought (~100 tokens) + tool call (~20 tokens) + observation (~200 tokens). Starting context = 500 tokens. Compute the worst case.',
        questions: [
          { label: 'Worst-case context at turn 10', answer: 3700, tolerance: 50, unit: 'tokens',
  ...
  // ── L8: Capstone ──────────────────────────────────────────────────────────
  {
    ...
    steps: [
      ...
      },
      {
        ...
        whys: [
          undefined,
          'The forward pass is deterministic, but decoding adds a sampling step after it — that is where the randomness lives.',
          'Sampling happens at every position, not just the first; the sequences diverge more with each token.',
          'Same seed, same sequence — but nothing here says the seed was fixed, and T=1.0 samples on every call.',
        ],
      ...
      {
        kind: 'numeric',
        prompt: 'Trace through a full inference call. Compute both answers.',
        code: '# GPT-2 small: 12 layers, 12 heads, hidden=768, vocab=50257\n# Prompt: "The model predicts" → 4 tokens\n# Generating 1 new token',
        questions: [
          { label: 'KV-cache tensors appended for this 1 new token (all layers)', answer: 24, tolerance: 0,
          ...
          { label: 'Logit vector length for the final hidden state', answer: 50257, tolerance: 0,
...
export const WARMUPS: Record<string, string> = {
  ...
  'c6-l1': 'rag',
  'c6-l2': 'finetuning',
  'c6-l3': 'generalisation',
  'c6-l4': 'alignment',
  'c6-d1': 'alignment',
  'c6-l6': 'generalisation',
  'c7-l1': 'capstone',
  'c7-l3': 'decoding',
  'c7-l4': 'agents',
  'c7-l5': 'capstone',
  ...
  'c7-l8': 'frontier',
  'c7-l6': 'capstone',
  'c8-l1': 'generalisation',
  'c8-l2': 'capstone',
  'c8-l3': 'generalisation',
  'c8-l4': 'modelcards',
  'c8-l5': 'capstone',
  'c8-d1': 'capstone',
  'c8-l6': 'capstone',
