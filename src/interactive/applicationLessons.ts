...
export const APPLICATION_LESSONS: InteractiveLesson[] = [
  ...
  },
  {
    ...
    steps:[
      ...
      {kind:'predict',prompt:'Choose retry behavior based on the failure type.',questions:[{label:'A tool returns a permanent validation error. What is the best behavior?',options:['Stop retrying and surface a controlled failure','Retry forever','Raise temperature','Delete prior context'],answer:0,reveal:'Retries help transient failures, not invalid requests. Infinite retries amplify cost and risk.'},{label:'A tool returns a transient timeout instead. What is the best behavior?',options:['Retry with a limit, then fail gracefully','Never retry','Permanently block the tool'],answer:0,reveal:'Transient errors may resolve on retry, but a cap prevents unbounded cost if the service stays down.'}]},
    ],
  },
  {
  ...
  },
  {
    ...
    steps:[
      ...
      {kind:'predict',prompt:'Choose a response when retrieved sources conflict.',questions:[{label:'Confidence is low and neither source is clearly authoritative. What is the best response?',options:['Acknowledge uncertainty, avoid the action, and escalate or ask for clarification','Invent a tie-breaker','Choose the longer passage','Increase temperature'],answer:0,reveal:'Safe fallbacks preserve uncertainty and prevent unsupported action.'},{label:'One passage is clearly more recent and authoritative. What is the best response?',options:['Prefer it but still note the conflict for the user','Average the two answers','Ignore both and generate freely'],answer:0,reveal:'Source recency and authority matter, but surfacing the conflict maintains trust and traceability.'}]},
